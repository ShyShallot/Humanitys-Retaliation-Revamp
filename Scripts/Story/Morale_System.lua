...
function Definitions()
    ...
    Random_Events = {
        ...
        Random_Chances = {
            ...
            Chance_Increase_Per_Tick = 0.0025,
            Chance_Cap = 0.5,
            Last_Happened = 0,
        }
    }
...
function Morale_System_Update(message)
    if message == OnUpdate then
        ...
        if GlobalValue.Get("Morale_Status") ~= Current_Morale_Status and GlobalValue.Get("Morale_Status") ~= nil then
            ...
            Display_Handler:Add_Body("TEXT_STORY_MORALE_DISPLAY_ALERT_" .. string.upper(Current_Morale_Entry.Name), 8, true, nil, nil)
        end
...
function Display_Handler:Add_Header(string, color, var, time)
    ...
    for _, sHeader in pairs(self.Body) do
...
---@param color? table
---@param var? any
function Display_Handler:Add_Footer(string, color, var)
    if type(string) ~= "string" then
        return
    end
    ...
    table.insert(self.Footer, {Text = string, Color = color, Var = var})
end
